...
#        analyze(X)
    ...
    with open('StockPrice.csv') as csvfile:
        ...
        for row in readCSV:
            score = mydict[row[0]]
            stockpricechange = float(row[1])*100
            row_new=[score,stockpricechange]
            if row[2]=='0':
    ...
    with open('Day1.csv','wb') as csvWriteFile:
        writeCSV=csv.writer(csvWriteFile,delimiter=",")
        writeCSV.writerows(Day1)
    with open('Day2.csv','wb') as csvWriteFile:
        writeCSV=csv.writer(csvWriteFile,delimiter=",")
        writeCSV.writerows(Day2)
